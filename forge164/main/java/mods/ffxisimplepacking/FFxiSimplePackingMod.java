...
//		SpiTools.printOutListName();
	...
	private void setGameRegistryBlocks(String text)
	{
		for (int i = 0; i < SpiSetting.spiTypeTableMax; i++)
		{
			...
			{
				SpiTools.setBlockInstance(text, i, (new BlockSimplePackingBlock(SpiSetting.SimplePackingBlockID + i, Material.glass, i))
						.setUnlocalizedName(SpiTools.getSpiName(text, i)).setTextureName(SpiSetting.imageBlock[i]).setCreativeTab(this.addCreativeTabs(text, i)));
				GameRegistry.registerBlock(SpiTools.getBlockInstance(text, i), ItemSimplePackingBlock.class, SpiTools.getSpiName(text, i));
			}
			else
			{
				SpiTools.setBlockInstance(text, i, (new BlockSimplePackingItem(SpiSetting.SimplePackingItemID + i, Material.glass, i))
						.setUnlocalizedName(SpiTools.getSpiName(text, i)).setTextureName(SpiSetting.imageBlock[i]).setCreativeTab(this.addCreativeTabs(text, i)));
	...
	private void setConfigrationfile(File file)
	{
		...
		try{
			config.load();
			SpiSetting.SimplePackingBlockID = config.getBlock("SimplePackingBlockID", SpiSetting.SimplePackingBlockID).getInt();
			SpiSetting.SimplePackingItemID = config.getBlock("SimplePackingItemID", SpiSetting.SimplePackingItemID).getInt();
		...
		{
			//Error Log Printout
			FMLLog.log(Level.SEVERE, e, MODID + " : error configuration file");
		}
		finally
		{
			config.save();
	...
	public void setRecipeOptionsBlock()
	{
		...
		{
			GameRegistry.addShapelessRecipe(new ItemStack(Block.sandStone,1,1)      , new Object[]{ new ItemStack(Block.sandStone,1,Short.MAX_VALUE), Block.dirt });
			GameRegistry.addShapelessRecipe(new ItemStack(Block.sandStone,1,2)      , new Object[]{ new ItemStack(Block.sandStone,1,Short.MAX_VALUE), Block.sand });
			GameRegistry.addShapelessRecipe(new ItemStack(Block.stoneBrick,1,1)     , new Object[]{ new ItemStack(Block.stoneBrick,1,Short.MAX_VALUE), Block.vine });
			GameRegistry.addShapelessRecipe(new ItemStack(Block.stoneBrick,1,2)     , new Object[]{ new ItemStack(Block.stoneBrick,1,Short.MAX_VALUE), Block.gravel });
			GameRegistry.addShapelessRecipe(new ItemStack(Block.stoneBrick,1,3)     , new Object[]{ new ItemStack(Block.stoneBrick,1,Short.MAX_VALUE), Block.dirt });
		}
	}
			...
			if (entry.getValue() != null && entry.getValue() != null && entry.getValue().getObject() != null && entry.getValue().getCraftObject() != null)
			{
				if(entry.getValue().isCraftItemBlock())
				{
					GameRegistry.addRecipe(new ItemStack(SpiTools.getBlockInstance(type, 0), 1, entry.getKey()),
							new Object[]{ SpiSetting.rp[0],SpiSetting.rp[1],SpiSetting.rp[2], Character.valueOf('X'), new ItemStack(entry.getValue().getCraftBlock().blockID, 1, entry.getValue().getMataData()) });
				...
				{
					GameRegistry.addRecipe(new ItemStack(SpiTools.getBlockInstance(type, 0), 1, entry.getKey()),
							new Object[]{ SpiSetting.rp[0],SpiSetting.rp[1],SpiSetting.rp[2], Character.valueOf('X'), new ItemStack(entry.getValue().getCraftItem().itemID, 1, entry.getValue().getMataData()) });
