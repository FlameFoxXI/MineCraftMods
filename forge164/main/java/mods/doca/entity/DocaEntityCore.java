...
public class DocaEntityCore extends EntityTameable
{
	...
	protected void entityInit()
	{
		...
		this.dataWatcher.addObject(25, new ItemStack(Item.bow));           //ItemStack
	...
	public void readEntityFromNBT(NBTTagCompound par1NBTTagCompound)
	{
		...
		NBTTagList tmpNBTTagList = par1NBTTagCompound.getTagList("Inventory");
		this.inventory.readFromNBT(tmpNBTTagList);
	...
	public ItemStack getCurrentItemData()
	{
		...
		if (tmpItemStack.itemID <= Item.itemsList.length && Item.itemsList[tmpItemStack.itemID] != null)
		{
			return tmpItemStack;
		}
		else
	...
	public void setCurrentItemData(ItemStack par1ItemStack)
	{
		...
		{
			this.dataWatcher.updateObject(25, new ItemStack(Item.bow));
		}
	}
	...
	private int[] convertStrToIntTable(String par1String, int tableSize)
	{
		...
		if (tmp.length == tmpInt.length)
		{
			for(int i = 0; i < tmpInt.length; i++)
			{
				...
				{
					DocaTools.wrning("dataWatcher string to convert integer error!!");
					e.printStackTrace();
				}
			}
		}
		else 
		{
			...
			DocaTools.wrning("dataWatcher string error!!");
		}
		return tmpInt;
	}
}
