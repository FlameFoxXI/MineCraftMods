...
public class DocaEntityBase extends DocaEntityCore
{
	...
	public boolean interact(EntityPlayer par1EntityPlayer)
	{
		...
		if (itemStack != null)
		{
			...
			else if (DocaTools.ofItem(itemStack, Item.compass))
			{
				if (!this.isTamed())
				{
					this.setTamed(true);
			...
			else if (DocaTools.ofItem(itemStack, Item.bone) && this.isTamed()
					&& (par1EntityPlayer.getCommandSenderName().equalsIgnoreCase(this.getOwnerName()) || DocaSet.DebugItem))
			{
				if (this.isSitting())
				{
					this.aiSit.setSitting(false);
			...
			else if (DocaTools.ofItem(itemStack, Item.sign) && this.isTamed()
					&& (par1EntityPlayer.getCommandSenderName().equalsIgnoreCase(this.getOwnerName()) || DocaSet.DebugItem))
			{
				if (!this.isWaiting())
				{
			...
			else if(DocaTools.ofItem(itemStack, Item.carrotOnAStick) && this.isTamed()
					&& (par1EntityPlayer.getCommandSenderName().equalsIgnoreCase(this.getOwnerName()) || DocaSet.DebugItem))
			{
				this.funcRiddenByEntity.interact(par1EntityPlayer, this, Item.carrotOnAStick);					
				return true;
			}
			else if (DocaTools.ofItem(itemStack, Item.feather) && this.isTamed()
					&& (par1EntityPlayer.getCommandSenderName().equalsIgnoreCase(this.getOwnerName()) || DocaSet.DebugItem))
			{
				DocaTools.openEntityGuiDoca(DocaSet.containerSettingID, this.worldObj, this.entityId, par1EntityPlayer);
				return true;
			}
			else if (DocaTools.ofItem(itemStack, Item.shears) && this.isTamed()
					&& (par1EntityPlayer.getCommandSenderName().equalsIgnoreCase(this.getOwnerName()) || DocaSet.DebugItem))
			{
				this.setName("");
				this.setMode(0);
			...
			else if(DocaTools.ofItem(itemStack, Item.stick) && (this.getMode() != 0 || DocaSet.DebugItem))
			{
				DocaTools.setDocaToSendQueue(this, par1EntityPlayer, DocaTools.PACKET_SOWN_DATA);

				if (par1EntityPlayer.getCommandSenderName().equalsIgnoreCase(this.getOwnerName()) || this.checkSubOwners(par1EntityPlayer.getCommandSenderName()) || DocaSet.Debug || DocaTools.isOps(par1EntityPlayer))
				{
					DocaTools.openEntityGuiDoca(DocaSet.containerContainerID, this.worldObj, this.entityId, par1EntityPlayer);
				}
				return true;
			}
		}
		...
		{
			if (this.isTamed() && (par1EntityPlayer.getCommandSenderName().equalsIgnoreCase(this.getOwnerName()) || DocaSet.DebugItem))
			{
	...
	public void statusUpdate()
	{
		if (this.isTamed())
		{
			...
			{
				if (true)//if (DocaSet.statusKeyplessUser.equalsIgnoreCase(this.getOwnerName()))
				{
	...
	{
		return this.funcRiddenByEntity.canBeSteered(((EntityPlayer)this.riddenByEntity).getHeldItem(), Item.carrotOnAStick);
	}
