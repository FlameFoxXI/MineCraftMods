...
public class DocaTools
{
	...
	public static Logger logger = Logger.getLogger("DogCatPlus");
	...
	public static void regConfigurationile(FMLPreInitializationEvent event)
	{
		...
		{
			DocaTools.wrning("DogCatPlus has a problem loading it's configuration");
		}
		finally
		{
			config.save();
	...
	{
		DocaTools.msDoca(Level.WARNING, message);
	}

	public static void info(String message)
	{
		DocaTools.msDoca(Level.INFO, message);
	}

	public static void severe(String message)
	{
		DocaTools.msDoca(Level.SEVERE, message);
	}

	public static void msDoca(Level level, String message)
	{
		if (DocaSet.Debug || DocaSet.DebugData || DocaSet.DebugData)
		{
			System.out.println(message);
		}
		else
		{
			DocaTools.logger.log(level, message);
		}
	}
	...
	{
		FMLNetworkHandler.openGui(par1EntityPlayer, getModInstance(), modGuiId, world, entityId, 0, 0);
	}
	...
	public static void setDocaToSendQueue(DocaEntityBase theBase, EntityPlayer player, String type)
	{
		...
		{
			new DocaPacketType(theBase).sendToServer();
		}
		else if (type.equalsIgnoreCase(PACKET_SOWN_DATA))
		{
			new DocaPacketSubOwner(theBase).sendToPlayer(player);
		}
	}
	...
	public static void setDocaToSendQueue(DocaKeys key, String type)
	{
		...
		{
			new DocaPacketKey(DocaSet.keyLocalDoca).sendToServer();
		}
	}
	...
	public static boolean ofItem(ItemStack baseitem, Object compItem)
	{
		...
		if (compItem instanceof Item && baseitem.getItem().itemID == ((Item)compItem).itemID)
		{
			return true;
		}
		else if (compItem instanceof Block && baseitem.getItem().itemID == ((Block)compItem).blockID)
		{
			return true;
		}
		else
	...
	public static boolean ofItem(Item baseitem, Object compItem)
	{
		...
		if (compItem instanceof Item && baseitem.itemID == ((Item)compItem).itemID)
		{
			return true;
		}
	...
	{
		return itemstack != null ? itemstack.itemID : 0;
	}
	...
	public static DocaEntityBase createEntity(int no, World par1World)
	{
		...
		{
			DocaTools.wrning("exception create entity. entity=" + entity + ", no=" + no);
			e.printStackTrace();
		}
	...
	public static void setLoginUser(String UserName)
	{
		...
		{
			DocaTools.wrning("ERROR: user name is empty. 0001");
		}
	}
	...
	public static DocaKeys getLoginUser(String UserName)
	{
		...
		{
			DocaTools.wrning("ERROR: user name is empty. 0002");
		}
		return null;
	}
	...
	public static void updateLoginUser(DocaKeys data)
	{
		...
		{
			DocaTools.wrning("ERROR: user name is empty. 0003");
		}
	}
