...
public class FFxiCallAnimalBellMod
{
	...
	public static String soundBell = imageFolda + "bell.animalbell";
	
	//config
	public static int itemAnimalBellId = 5200;
	...
	public void preInit(FMLPreInitializationEvent event)
	{
		...
		itemAnimalBell = (new ItemCallAnimalBell(itemAnimalBellId)).setUnlocalizedName("bellAnimal").setTextureName(imageFolda + "animal_bell");
	...
	{
		String regSound = soundBell.replace(".", "/") + ".ogg";
		try
		{
			event.manager.soundPoolSounds.addSound(regSound);
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	private void repEntityAIAnimalBell(EntityPlayer player)
	{
		...
		if (list != null && !list.isEmpty())
		{
			...
			while (iterator.hasNext())
			{
	            ...
	            List<EntityAITaskEntry> tasks = entity.tasks.taskEntries;
	...
	private void configrationfile(File file)
	{
		...
		try
		{
			...
			itemAnimalBellId = config.getItem("itemAnimalBellId", itemAnimalBellId, "item Animal Bell Id.").getInt();
	...
	private void addItemRecipe()
	{
		GameRegistry.addRecipe(new ItemStack(itemAnimalBell, 1),
				new Object[] { " F ", " S ", " G ", 'F', Item.feather, 'S', Item.stick, 'G', Item.ingotGold });
	}
	...
	{
		world.playSoundAtEntity(entity, name, volume, pich);
	}
}