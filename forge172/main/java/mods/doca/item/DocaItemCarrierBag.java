		...
		if (!par2World.isRemote)
		{
			...
			if (tmpNBTTagCompound != null)
			{
				...
				if (!this.petTypeString.equalsIgnoreCase(""))
				{
					...
					if (DocaSet.Debug)
					{
						this.petOwnerName = par3EntityPlayer.getCommandSenderName();
					}
					
					if (par3EntityPlayer.getCommandSenderName().equalsIgnoreCase(this.petOwnerName))
					{
						...
						for(int i = 0; i < DocaReg.getSettingsMax(); i++)
						{
							if(DocaReg.getTypeDisp(i).equalsIgnoreCase(this.petTypeString) && DocaReg.getUse(i))
							{
								...
								tmpEntity.setOwner(this.petOwnerName);
	...
	public boolean hitEntity(ItemStack par1ItemStack, EntityLivingBase par2EntityLivingBase, EntityLivingBase par3EntityLivingBase)
	{
		...
		if (!par3EntityLivingBase.worldObj.isRemote)
		{
			if (par2EntityLivingBase != null && par2EntityLivingBase instanceof DocaEntityBase)
			{
				...
				if (tmpEntityPlayer.getCommandSenderName().equalsIgnoreCase(tmpDocaEntityBase.getOwnerName()) && DocaReg.getUse(DocaReg.getNameToType(tmpDocaEntityBase.getEntityDocaString())))
				{
					if (this.petTypeString.equalsIgnoreCase(""))
					{
						...
						this.petOwnerName = tmpDocaEntityBase.getOwnerName();
